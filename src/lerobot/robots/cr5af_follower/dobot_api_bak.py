...
MyType = np.dtype([
    ('len', np.uint16,),                   # 数据包长度
    ('reserve', np.byte, (6, )),           # 保留字节，未使用
    ('DigitalInputs', np.uint64,),         # 数字输入状态
    ('DigitalOutputs', np.uint64,),        # 数字输出状态
    ('RobotMode', np.uint64,),             # 机器人模式
    ('TimeStamp', np.uint64,),             # 时间戳
    ('RunTime', np.uint64,),               # 系统运行时间
    ('TestValue', np.uint64,),             # 测试值
    ('reserve2', np.byte, (8, )),          # 保留字节
    ('SpeedScaling', np.float64,),         # 速度缩放比例
    ('reserve3', np.byte, (16, )),         # 保留字节
    ('VRobot', np.float64, ),              # 机器人电压
    ('IRobot', np.float64,),               # 机器人电流
    ('ProgramState', np.float64,),         # 程序运行状态
    ('SafetyOIn', np.uint16,),             # 安全输入
    ('SafetyOOut', np.uint16,),            # 安全输出
    ('reserve4', np.byte, (76, )),         # 保留字节
    ('QTarget', np.float64, (6, )),        # 关节目标角度
    ('QDTarget', np.float64, (6, )),       # 关节目标速度
    ('QDDTarget', np.float64, (6, )),      # 关节目标加速度
    ('ITarget', np.float64, (6, )),        # 关节目标电流
    ('MTarget', np.float64, (6, )),        # 关节目标力矩
    ('QActual', np.float64, (6, )),        # 关节实际角度！！！！！
    ('QDActual', np.float64, (6, )),       # 关节实际速度
    ('IActual', np.float64, (6, )),        # 关节实际电流
    ('ActualTCPForce', np.float64, (6, )), # 实际 TCP 力（末端受力）
    ('ToolVectorActual', np.float64, (6,)),# 工具实际位姿
    ('TCPSpeedActual', np.float64, (6, )), # TCP 实际速度
    ('TCPForce', np.float64, (6, )),       # TCP 受力
    ('ToolVectorTarget', np.float64, (6,)),# 工具目标位姿
    ('TCPSpeedTarget', np.float64, (6, )), # TCP 目标速度
    ('MotorTemperatures', np.float64, (6,)),# 电机温度
    ('JointModes', np.float64, (6, )),     # 关节模式
    ('VActual', np.float64, (6, )),        # 实际电压
    ('HandType', np.byte, (4, )),          # 末端手爪类型
    ('User', np.byte,),                    # 用户索引
    ('Tool', np.byte,),                    # 工具索引
    ('RunQueuedCmd', np.byte,),            # 运行队列命令标志
    ('PauseCmdFlag', np.byte,),            # 暂停命令标志
    ('VelocityRatio', np.byte,),           # 速度比例
    ('AccelerationRatio', np.byte,),       # 加速度比例
    ('reserve5', np.byte,),                # 保留字节
    ('XYZVelocityRatio', np.byte,),        # XYZ 平移速度比例
    ('RVelocityRatio', np.byte,),          # R 旋转速度比例
    ('XYZAccelerationRatio', np.byte,),    # XYZ 平移加速度比例
    ('RAccelerationRatio', np.byte,),      # R 旋转加速度比例
    ('reserve6', np.byte,(2,)),            # 保留字节
    ('BrakeStatus', np.byte,),             # 制动状态
    ('EnableStatus', np.byte,),            # 使能状态
    ('DragStatus', np.byte,),              # 拖动状态
    ('RunningStatus', np.byte,),           # 运行状态
    ('ErrorStatus', np.byte,),             # 错误状态
    ('JogStatusCR', np.byte,),             # 点动状态
    ('CRRobotType', np.byte,),             # 机器人类型
    ('DragButtonSignal', np.byte,),        # 拖动按钮信号
    ('EnableButtonSignal', np.byte,),      # 使能按钮信号
    ('RecordButtonSignal', np.byte,),      # 记录按钮信号
    ('ReappearButtonSignal', np.byte,),    # 回放按钮信号
    ('JawButtonSignal', np.byte,),         # 夹爪按钮信号
    ('SixForceOnline', np.byte,),          # 六维力传感器是否在线
    ('CollisionState', np.byte,),          # 碰撞检测状态
    ('ArmApproachState', np.byte,),        # 大臂接近状态
    ('J4ApproachState', np.byte,),         # 腕部-第四关节接近状态
    ('J5ApproachState', np.byte,),         # 腕部-第五关节接近状态
    ('J6ApproachState', np.byte,),         # 腕部-第六关节接近状态
    ('reserve7', np.byte, (61, )),         # 保留字节
    ('VibrationDisZ', np.float64,),        # Z 方向振动位移
    ('CurrentCommandId', np.uint64,),      # 当前命令 ID
    ('MActual', np.float64, (6, )),        # 实际关节力矩
    ('Load', np.float64,),                 # 末端负载
    ('CenterX', np.float64,),              # 负载质心 X
    ('CenterY', np.float64,),              # 负载质心 Y
    ('CenterZ', np.float64,),              # 负载质心 Z
    ('UserValue[6]', np.float64, (6, )),   # 用户自定义值
    ('ToolValue[6]', np.float64, (6, )),   # 工具自定义值
    ('reserve8', np.byte, (8, )),          # 保留字节
    ('SixForceValue', np.float64, (6, )),  # 六维力传感器值
    ('TargetQuaternion', np.float64, (4,)),# 目标姿态四元数
    ('ActualQuaternion', np.float64, (4,)),# 实际姿态四元数
    ('AutoManualMode', np.uint16, ),       # 自动/手动模式
    ('ExportStatus', np.uint16, ),         # 导出状态
    ('SafetyState', np.byte, ),            # 安全状态
    ('reserve9', np.byte,(19,))            # 保留字节
])
...
class DobotApiDashboard(DobotApi):
    ...
    def SetTool485(self, index, parity='', stopbit=-1, identify=-1):
        ...
        # if string != -1:
        if stopbit != -1:                     # ← 原来写成了 if string != -1
    ...
    def GetOutputFloat(self, address):
        ...
        """
        # string = "GetInputFloat({:d})".format(address)
        string = "GetOutputFloat({:d})".format(address)  # ← 修成这样
    ...
    def SetOutputBool(self, address, value):
        ...
        string = "SetOutputBool({:d},{:d})".format(address, value)  # ← 修成这样
...
class DobotDemo:
    ...
    def start(self):
        ...
        # point_a = [-76.6622,426.5791,86.9635,90.2519,-2.9989,170.9939]  # 打包姿态
        # point_a = [18.6270,-411.2508,1093.3549,-89.9838,-0.0001,-179.7240]  # 安全原点
        point_a = [18.6270,-411.2508,1093.3549,-89.9838,-0.0001,-179.7240]  # 安全原点
        point_b = [146.3759,-283.4321,432.3956,177.7879,-1.8540,147.5821]

        # 关节角
        joint_a = [0,0,37.7600,0,0,0]


        # 启动走点线程
        runpoint_thread = threading.Thread(
            target=self.RunPoint(joint_a))  
        runpoint_thread.daemon = True
        runpoint_thread.start()

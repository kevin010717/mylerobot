...
import sys, tty, termios, select, threading, random, re
from time import sleep
class DobotDemo1:
    def __init__(self, ip):
        self.ip = ip
        ...
        self.feedData = FeedItem()
        self.__globalLockValue = threading.Lock()

        # 控制参数
        self.joint_pos = [0, 0, 90, 0, -90, 0]   # 6关节角度
        self.selected = 0                     # 当前选中的关节
        self.step = 5.0                       # 每次调节的角度 (deg)
        self.running = True
    ...
    def start(self):
        ...
        # 键盘线程
        threading.Thread(target=self.KeyboardLoop, daemon=True).start()

        # 主循环：打印状态
        while self.running:
            print("关节角度:", self.joint_pos, "模式:", self.feedData.robotMode)
            sleep(1)
    ...
    def GetFeed(self):
        while True:
            feedInfo = self.feedFour.feedBackData()
            with self.__globalLockValue:
                if feedInfo is not None and hex(feedInfo['TestValue'][0]) == '0x123456789abcdef':
                    self.feedData.MessageSize = feedInfo['len'][0]
                    self.feedData.robotMode = feedInfo['RobotMode'][0]
                    self.feedData.DigitalInputs = feedInfo['DigitalInputs'][0]
                    self.feedData.DigitalOutputs = feedInfo['DigitalOutputs'][0]
    ...
    def handle_key(self, ch):
        ...
        if ch == 'k':
            ...
            return
        if ch == '[':
            self.step = max(1, self.step/1.5)
            print(f"步长减小: {self.step:.2f}°")
            return
        if ch == ']':
            self.step = min(30, self.step*1.5)
            print(f"步长增大: {self.step:.2f}°")
            return
        if ch == 'r':
            self.joint_pos = [0,0,0,0,0,0]
            self.joint_pos = [0, 0, 90, 0, -90, 0]
            self.move_joint()
            print("重置所有关节为 0°")
            return
        if ch == 'z':
...
#     """
#     Windows 版键盘控制 Demo：
#       - 不再依赖 tty/termios/select
#       - 使用 msvcrt.kbhit()/getwch() 非阻塞读取键盘
#       - 保持与原始 Demo 基本一致的热键映射
#     """
#     def __init__(self, ip):
#         self.ip = ip
#         self.dashboardPort = 29999
...
#         # 控制参数
#         self.joint_pos = [0, 0, 0, 0, 0, 0]  # 6 关节角度 (deg)
#         self.selected = 0                    # 当前选中的关节索引 [0..5]
#         self.step = 5.0                      # 每次调节的角度 (deg)
#         self.running = True
...
#         enable_ret = self.dashboard.EnableRobot()
#         if self.parseResultId(enable_ret)[0] != 0:
#             print("使能失败: 请检查 29999 端口 或 机器人状态")
#             return
#         print("使能成功")
...
#             while self.running:
#                 with self.__globalLockValue:
#                     mode = self.feedData.robotMode
#                 print(f"关节角度: {['%.2f' % x for x in self.joint_pos]} | 当前关节: {self.selected+1} | 步长: {self.step:.2f}° | 模式: {mode}")
#                 sleep(1)
...
#                     continue
#                 # 校验示例（根据你们的协议可调整）
#                 if 'TestValue' in feedInfo and hex(feedInfo['TestValue'][0]) == '0x123456789abcdef':
#                     with self.__globalLockValue:
#                         self.feedData.MessageSize = feedInfo.get('len', [None])[0]
#                         self.feedData.robotMode = feedInfo.get('RobotMode', [-1])[0]
#                         self.feedData.DigitalInputs = feedInfo.get('DigitalInputs', [-1])[0]
#                         self.feedData.DigitalOutputs = feedInfo.get('DigitalOutputs', [-1])[0]
#                         self.feedData.robotCurrentCommandID = feedInfo.get('CurrentCommandId', [-1])[0]
#             except Exception as e:
#                 # 避免线程崩溃
#                 print("GetFeed 异常：", e)
#                 sleep(0.05)
...
#         Windows 非阻塞键盘读取：
#           - msvcrt.kbhit(): 是否有按键
#           - msvcrt.getwch(): 读一个宽字符（不回显）
#         """
#         while self.running:
#             try:
#                 if msvcrt.kbhit():
#                     ch = msvcrt.getwch()  # 宽字符，能处理中英文/符号
#                     # 处理功能键（如方向键会先返回 '\xe0' 或 '\x00' 前缀）
#                     if ch in ('\xe0', '\x00'):  # 丢弃前缀并读真正的键码
#                         _ = msvcrt.getwch()
#                         continue
#                     self.handle_key(ch)
#                 else:
...
#     def move_joint(self):
#         try:
#             # MovJ(j1, j2, j3, j4, j5, j6, 速度或其他参数)
#             recvmovemess = self.dashboard.MovJ(*self.joint_pos, 1)
#             print("发送 MovJ:", recvmovemess)
#         except Exception as e:
#             print("发送 MovJ 异常：", e)

#     def parseResultId(self, valueRecv: str):
#         if isinstance(valueRecv, str) and "Not Tcp" in valueRecv:
#             print("Control Mode Is Not Tcp")
#             return [1]
#         nums = re.findall(r'-?\d+', valueRecv) if isinstance(valueRecv, str) else []
...
if __name__ == '__main__':
    ...
    # dobot = WinDobotDemo("192.168.201.1")
    # dobot.start()
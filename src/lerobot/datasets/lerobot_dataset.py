...
class LeRobotDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        timestamps = torch.stack([self.hf_dataset[i]["timestamp"]
                          for i in range(len(self.hf_dataset))]).numpy()
        # timestamps_np = timestamps.numpy()
        # Check timestamps
        # timestamps = torch.stack(self.hf_dataset["timestamp"]).numpy()  # 有坑
        # episode_indices = torch.stack(self.hf_dataset["episode_index"]).numpy()
        timestamps = torch.stack([self.hf_dataset[i]["timestamp"]
                    for i in range(len(self.hf_dataset))]).numpy()
        episode_indices = torch.stack([self.hf_dataset[i]["episode_index"]
            for i in range(len(self.hf_dataset))]).numpy()
    ...
    #     return {
    #         key: torch.stack(self.hf_dataset.select(q_idx)[key]) # 有坑
    ...
    def _query_hf_dataset(self, query_indices: dict[str, list[int]]) -> dict:
        ...
        for key, q_idx in query_indices.items():
            ...
            # 逐行取，触发 set_transform，保证返回 torch.Tensor
            vals = []
            for i in q_idx:
                v = self.hf_dataset[i][key]
                if v is None:
                    continue

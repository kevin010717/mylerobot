...
def train(cfg: TrainPipelineConfig):
    ...
    dataset,
    num_workers=0,               # 临时：0
    pin_memory=False,            # 临时：False
    persistent_workers=False,    # 临时：False
    batch_size=cfg.batch_size,
    shuffle=shuffle,
    sampler=sampler,
    drop_last=False,
    ...
    print({k: (v.shape, v.dtype) for k, v in batch.items() if torch.is_tensor(v)})
